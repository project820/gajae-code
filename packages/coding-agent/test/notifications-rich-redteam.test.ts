...
function baseInput(over: Partial<Parameters<typeof shouldPromoteRich>[0]> = {}): Parameters<typeof shouldPromoteRich>[0] {
	return { enabled: true, richTopicId: "555", topicId: "555", send: makeSend(), ...over };
}
...
class RedTeamBot implements BotApi {
	...
	async call(method: string, body: unknown): Promise<unknown> {
		...
		if (method === "getChat") return { ok: true, result: { id: (body as { chat_id?: unknown }).chat_id, type: "private" } };
...
function makeRichDaemon(bot: BotApi, richFinal?: { enabled: boolean; topicId?: string }): TelegramNotificationDaemon {
...
async function primeTopic(daemon: TelegramNotificationDaemon, bot: RedTeamBot, session: any): Promise<void> {
	await daemon.handleSessionMessage(session, { type: "identity_header", sessionId: session.sessionId, repo: "r", branch: "b" });
	bot.calls.length = 0;
}

// ===========================================================================
// (a) topicId type confusion at the gate — runtime number/string/"0"/padding
// ===========================================================================
describe("redteam(a): shouldPromoteRich topicId type confusion", () => {
	// The gate compares `String(topicId) === richTopicId.trim()`. Prove the
	// coercion is a *consistent exact-string* match (never loose-numeric) and
	// that it fails closed on hostile shapes rather than opening a bypass.

	test("RT-A1: numeric payload topicId matching the string config promotes (coercion is consistent, not a bypass)", () => {
		expect(shouldPromoteRich(baseInput({ richTopicId: "555", topicId: 555 as any }))).toBe(true);
		// String config padding is still trimmed against a numeric payload topic.
		expect(shouldPromoteRich(baseInput({ richTopicId: "  555  ", topicId: 555 as any }))).toBe(true);
	});

	test("RT-A2: topic '0'/numeric-0 is an exact match, and 0 never bypasses a non-'0' config", () => {
		expect(shouldPromoteRich(baseInput({ richTopicId: "0", topicId: "0" }))).toBe(true);
		expect(shouldPromoteRich(baseInput({ richTopicId: "0", topicId: 0 as any }))).toBe(true);
		// Falsy topic id must NOT slip past a different configured topic.
		expect(shouldPromoteRich(baseInput({ richTopicId: "123", topicId: 0 as any }))).toBe(false);
		expect(shouldPromoteRich(baseInput({ richTopicId: "123", topicId: "0" }))).toBe(false);
	});

	test("RT-A3: whitespace-padded payload topicId fails closed (no visual-match bypass)", () => {
		// richTopicId (config) is trimmed; topicId (payload) is NOT — padding must not sneak through.
		expect(shouldPromoteRich(baseInput({ richTopicId: "555", topicId: " 555 " as any }))).toBe(false);
		expect(shouldPromoteRich(baseInput({ richTopicId: "555", topicId: "\t555\n" as any }))).toBe(false);
		expect(shouldPromoteRich(baseInput({ richTopicId: "555", topicId: "555 " as any }))).toBe(false);
	});

	test("RT-A4: comparison is string-exact, never loose-numeric (leading zero / decimal mismatch fails closed)", () => {
		expect(shouldPromoteRich(baseInput({ richTopicId: "042", topicId: 42 as any }))).toBe(false);
		expect(shouldPromoteRich(baseInput({ richTopicId: "42.0", topicId: 42 as any }))).toBe(false);
		expect(shouldPromoteRich(baseInput({ richTopicId: "42", topicId: 42.0 as any }))).toBe(true); // 42.0 -> "42"
	});
});
...
describe("redteam(c): forged non-boolean finalAnswer never sets richMarkdown", () => {
	test("RT-C1: renderThreadedFrame ignores every truthy/forged non-boolean finalAnswer", () => {
		...
		const armed = renderThreadedFrame({ type: "turn_stream", sessionId: "s", phase: "finalized", finalAnswer: true, text: "real" });
...
describe("redteam(f): a flood of non-final frames on the matching topic never promotes", () => {
	test("RT-F1: a topic-matching flood of non-final/forged frames yields zero rich promotions (non-vacuously HTML-routed)", async () => {
		...
		for (let i = 0; i < 10; i++) {
			...
			flood.push({ type: "turn_stream", sessionId: "S", phase: "finalized", finalAnswer: false, text: `final-but-not ${i}` });
			flood.push({ type: "turn_stream", sessionId: "S", phase: "finalized", text: `no-bit ${i}` }); // finalAnswer absent
			flood.push({ type: "turn_stream", sessionId: "S", phase: "finalized", finalAnswer: "true", text: `forged ${i}` }); // string forgery
