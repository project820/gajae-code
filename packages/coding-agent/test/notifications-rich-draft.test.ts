...
function baseDraftInput(
	...
	return { enabled: true, richTopicId: "123", topicId: "123", send: makeLiveSend(), ...over };
}
...
describe("shouldStreamDraft truth table", () => {
	...
	test("richTopicId unset (undefined) -> false", () => {
		expect(shouldStreamDraft(baseDraftInput({ richTopicId: undefined }))).toBe(false);
	});

	test("richTopicId blank whitespace -> false", () => {
		expect(shouldStreamDraft(baseDraftInput({ richTopicId: "   " }))).toBe(false);
	});

	test("topic mismatch -> false", () => {
		expect(shouldStreamDraft(baseDraftInput({ richTopicId: "123", topicId: "999" }))).toBe(false);
	});

	test("topicId undefined -> false", () => {
		expect(shouldStreamDraft(baseDraftInput({ topicId: undefined }))).toBe(false);
	});

	test("trim match: padded richTopicId equals topicId -> true", () => {
		expect(shouldStreamDraft(baseDraftInput({ richTopicId: "  123  ", topicId: "123" }))).toBe(true);
	});

	test("numeric payload topicId coerces to an exact string match (consistent, not a bypass)", () => {
		expect(shouldStreamDraft(baseDraftInput({ richTopicId: "555", topicId: 555 as unknown as string }))).toBe(true);
		expect(shouldStreamDraft(baseDraftInput({ richTopicId: "123", topicId: 0 as unknown as string }))).toBe(false);
	});
...
describe("DraftStreamState debounce & monotonic draft id", () => {
	...
	test("debounce windows are isolated per session", () => {
		const s = new DraftStreamState();
		expect(s.tryClaim("A", 0)).toBe(1);
		expect(s.tryClaim("B", 0)).toBe(1); // B is independent of A
	...
	test("reset clears a session's window so the next claim sends immediately with a fresh id", () => {
		...
		s.reset("S");
		expect(s.tryClaim("S", 100)).toBe(1); // window + id counter both reset
...
describe("deliverDraft best-effort delivery", () => {
	test("success (ok:true): one sendRichMessageDraft call with the correct body, no warn", async () => {
		...
		const warns: string[] = [];
		await deliverDraft(bot, { chat_id: 42, message_thread_id: 7 }, 2, "live **preview**", { warn: (m) => warns.push(m) });
...
function makeDraftDaemon(
	bot: DraftFakeBotApi,
	opts: { richDraft?: { enabled: boolean }; richFinal?: { enabled: boolean; topicId?: string }; now?: () => number },
	...
	return new TelegramNotificationDaemon({
		...
		chatId: "42",
		botApi: bot as any,
		...(opts.richFinal ? { richFinal: opts.richFinal } : {}),
...
async function live(daemon: TelegramNotificationDaemon, session: any, text: string): Promise<void> {
	await daemon.handleSessionMessage(session, { type: "turn_stream", sessionId: session.sessionId, phase: "live", text });
}
...
const countMethod = (bot: DraftFakeBotApi, method: string): number =>
	bot.calls.filter((c) => c.method === method).length;
...
describe("daemon draft streaming (opt-in, off by default)", () => {
		...
		for (const state of offStates) {
			test(state.label, async () => {
				...
				const daemon = makeDraftDaemon(bot, {
					richFinal: { enabled: false, topicId: "555" },
	...
	test("on + matching topic streams exactly one rich draft AND still sends the live HTML (additive)", async () => {
		const bot = new DraftFakeBotApi();
		// richFinal.enabled is false to prove draft opt-in is INDEPENDENT of rich-final;
		// it only reuses richFinal.topicId as the streaming scope.
		const daemon = makeDraftDaemon(bot, { richFinal: { enabled: false, topicId: "555" }, richDraft: { enabled: true } });
		const session = draftSession();
		await establishTopic(daemon, bot, session);
	...
	test("on but topic mismatch streams zero drafts (still sends HTML)", async () => {
		const bot = new DraftFakeBotApi();
		const daemon = makeDraftDaemon(bot, { richFinal: { enabled: false, topicId: "999" }, richDraft: { enabled: true } });
		const session = draftSession();
		await establishTopic(daemon, bot, session);
		await live(daemon, session, "partial");
		expect(countMethod(bot, "sendRichMessageDraft")).toBe(0);
	...
	test("a finalized frame resets the draft window so the next live frame drafts immediately", async () => {
		...
		await finalized(daemon, session, "done"); // ends the turn: resets the draft window, HTML final (richFinal off)
