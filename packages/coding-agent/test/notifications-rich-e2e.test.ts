 ...
 */
async function connectRealPipeline(richFinal?: { enabled: boolean; topicId?: string }): Promise<Harness> {
	...
	try {
		...
		await waitFor(() => daemon.sessions.has(sessionId) && srv.clientCount() >= 1, 8000, "daemon WS connect to napi server");
	} catch (err) {
		await stop();
		throw err;
	}
...
test("rich e2e: NotificationServer + real WS + daemon scanRoots -> finalAnswer promotes to sendRichMessage", async () => {
	const h = await connectRealPipeline({ enabled: true, topicId: String(THREAD_ID) });
	try {
		...
		h.srv.pushFrame(
			JSON.stringify({ type: "turn_stream", sessionId: h.sessionId, phase: "finalized", finalAnswer: true, text: RICH_TEXT }),
		);
...
test("rich e2e (off): a finalAnswer frame over the real WS stays on the HTML sendMessage path", async () => {
	const h = await connectRealPipeline({ enabled: false, topicId: String(THREAD_ID) });
	try {
...
test("rich e2e (lead-in): a finalized turn with finalAnswer:false is not promoted (the bit gates rich, not phase)", async () => {
	...
	try {
		...
		h.srv.pushFrame(
			JSON.stringify({ type: "turn_stream", sessionId: h.sessionId, phase: "finalized", finalAnswer: false, text: RICH_TEXT }),
		);
